...
import { adapterHandlers, runSourceAdapter } from "@mojis/adapters";
import {
  getAllEmojiVersions,
...
export async function buildEmojiGenerateRequest(
  ...
  if (isGeneratorEnabled("metadata", generators)) {
    const { groups, emojis } = await runSourceAdapter(adapterHandlers.metadataHandler, {
      force,
  ...
  if (isGeneratorEnabled("sequences", generators)) {
    const { sequences, zwj } = await runSourceAdapter(adapterHandlers.sequencesHandler, {
      force,
      emoji_version: version.emoji_version,
      unicode_version: version.unicode_version,
    });
  ...
  if (isGeneratorEnabled("variations", generators)) {
    const variations = await runSourceAdapter(adapterHandlers.variationsHandler, {
      force,
      emoji_version: version.emoji_version,
      unicode_version: version.unicode_version,
    });
  ...
  if (isGeneratorEnabled("unicode-names", generators)) {
    const unicodeNames = await runSourceAdapter(adapterHandlers.unicodeNamesHandler, {
      force,
      emoji_version: version.emoji_version,
      unicode_version: version.unicode_version,
    });
